...
class BGP(object):
    ...
    def _bgp_vrf_remote_as(self, rbridge_id, afi, vrf, addr, remote_as):
        ...
        bgp_vrf_remote_as = bgp_vrf_remote_as.format(addr=addr,
                                                     remote_as=remote_as)
    ...
    def local_asn(self, **kwargs):
        ...
        if is_get_config:
            ...
            api = 'rbridge_id_get'
            config = util.get_bgp_api(api=api,
                                      vrf=vrf,
                                      op='_get',
                                      afi=afi,
        ...
        config = util.get_bgp_api(vrf=vrf, afi=afi,
                                  rbridge_id=rbridge_id,
                                  args=args)
    ...
    def _neighbor_ip_address(self, afi='ipv4', n_addr=None, rbridge_id=1, remote_as='1',
                             vrf='default', callback=None, op='create'):
        if afi == 'ipv4':
            if vrf == 'default':
                self._neighbor_ipv4_address(afi, n_addr, rbridge_id, remote_as, callback, op)
            else:
                self._neighbor_ipv4_vrf_address(afi, n_addr, rbridge_id, remote_as, vrf, callback,
                                                op)
        if afi == 'ipv6':
            if vrf == 'default':
                self._neighbor_ipv6_address(afi, n_addr, rbridge_id, remote_as, callback, op)
            else:
                self._neighbor_ipv6_vrf_address(afi, n_addr, rbridge_id, remote_as, vrf, callback,
                                                op)

    def _neighbor_ipv4_address(self, afi='ipv4', n_addr=None, rbridge_id=1, remote_as='1',
        ...
        api = \
            'rbridge_id_router_bgp_address_family_ipv4_unicast_neighbor_af_ipv4_neighbor_address_create'
        config = (api, args)
        ret = callback(config)
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, afi=afi,
                                  feature='_neighbor_af_ipv4_neighbor_address',
                                  rbridge_id=rbridge_id, args=args)
        return callback(config)

    def _neighbor_ipv6_address(self, afi='ipv6', n_addr=None, rbridge_id=1, remote_as='1',
                               callback=None, op='create'):
    ...
    def _neighbor_ipv4_vrf_address(self, afi='ipv4', n_addr=None, rbridge_id=1, remote_as='1',
        ...
        api = \
            'rbridge_id_router_bgp_address_family_ipv4_unicast_vrf_neighbor_af_ipv4_neighbor_addr_create'
        config = (api, args)
        ret = callback(config)
        args = dict(rbridge_id=rbridge_id, af_vrf=vrf, af_ipv4_neighbor_addr=n_addr,
                    remote_as=remote_as)
        api = \
            'rbridge_id_router_bgp_address_family_ipv4_unicast_vrf_neighbor_af_ipv4_neighbor_addr_update'
        config = (api, args)
        ret = callback(config)
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, vrf=vrf, afi=afi,
                                  feature='_neighbor_af_ipv4_neighbor_addr', rbridge_id=rbridge_id,
                                  args=args)
        return callback(config)

    def _neighbor_ipv6_vrf_address(self, afi='ipv6', n_addr=None, rbridge_id=1, remote_as='1',
                                   vrf=None, callback=None, op='create'):
        self._bgp_vrf_remote_as(rbridge_id, afi, vrf, n_addr, remote_as)
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, vrf=vrf, afi=afi,
                                  feature='_neighbor_af_ipv6_neighbor_addr', rbridge_id=rbridge_id,
                                  args=args)
        return callback(config)
    ...
    def _neighbor_activate(self, n_addr, afi, rbridge_id, vrf, callback):
        ...
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, afi=afi, rbridge_id=rbridge_id, feature=feature,
                                  vrf=vrf, args=args)
        return callback(config)
    ...
    def get_bgp_neighbors(self, **kwargs):
        ...
        result = []
        if vrf is 'default':
            config = util.get_bgp_api(rbridge_id=rbridge_id, op='_get', resource_depth=2,
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi='ipv4', vrf=vrf, op='_get',
                                  feature=feature, resource_depth=2, args=dict())
        output = callback(config, handler='get_config')
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi='ipv6', vrf=vrf, op='_get',
                                  feature=feature, resource_depth=2, args=dict())
        output = callback(config, handler='get_config')
    ...
    def redistribute(self, **kwargs):
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_update',
    ...
    def max_paths(self, **kwargs):
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature,
                                  args=args)
        return callback(config)
    ...
    def multihop(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature,
                                      n_addr=str(ip_addr.ip), op='_get')
            output = callback(config, handler='get_config')
            output = util.findall(output.json, '$..ebgp-multihop-count')
            return output
        if kwargs.pop('delete', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature,
                                      n_addr=str(ip_addr.ip), op='_delete')
            return callback(config)
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature,
                                  n_addr=str(ip_addr.ip), args=args)
        return callback(config)
    ...
    def update_source(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature,
                                      n_addr=str(ip_addr.ip), args=args, op='_get')
            ret = callback(config)
            import pdb
            pdb.set_trace()
            return ret
    ...
    def evpn_afi_peer_activate(self, **kwargs):
        ...
        if kwargs.pop('delete', False):
            ...
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=peer_ip,
                                      op='_update', args=args)
            return callback(config)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=peer_ip,
                                      resource_depth=2, op='_get')
            ret = callback(config, handler='get_config')
    ...
    def bfd(self, **kwargs):
        ...
        if delete:
            ...
            return callback(config)
        if get:
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, resource_depth=2,
                                      op='_get')
            ret = callback(config, handler='get_config')
    ...
    def retain_rt_all(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            ...
            return ret
        args = dict(all=True)
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, afi=afi, op='_update',
                                  args=args)
        return callback(config)
    ...
    def evpn_next_hop_unchanged(self, **kwargs):
        ...
        if kwargs.pop('delete', False):
            ...
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, afi=afi, op='_update',
                                      evpn_n_addr=ip_addr, args=args)
            return callback(config)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=ip_addr,
                                      afi=afi, op='_get')
    ...
    def evpn_allowas_in(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=ip_addr,
                                      afi=afi, op='_get', resource_depth=2)
            out = callback(config, handler='get_config')
    ...
    def af_ip_allowas_in(self, **kwargs):
        ...
        if vrf == 'default':
            if 'ipv4' == afi:
                feature = feature_tmp.format('_address_family_ipv4_unicast',
                                             '_af_ipv4_neighbor_address')
            elif 'ipv6' == afi:
                feature = feature_tmp.format('_address_family_ipv6_unicast',
            ...
            feature = feature_tmp.format('', '_af_ipv6_neighbor_addr')
        if kwargs.pop('get', False):
            config = util.get_bgp_api(vrf=vrf, n_addr=str(ip_addr), rbridge_id=rbridge_id,
                                      op='_get', afi=afi, feature=feature, resource_depth=2)
            out = callback(config, handler='get_config')
            ret = util.findall(out.json, '$..allowas-in')[0]
            return ret
        if kwargs.pop('delete', False):
            feature = feature + '_allowas_in'
            config = util.get_bgp_api(vrf=vrf, n_addr=str(ip_addr), rbridge_id=rbridge_id, afi=afi,
                                      feature=feature)
            return callback(config)
        args = dict(allowas_in=allowas_in)
        config = util.get_bgp_api(vrf=vrf, n_addr=str(ip_addr), rbridge_id=rbridge_id, op='_update',
                                  afi=afi, feature=feature, args=args)
        return callback(config)
    ...
    def peer_bfd_timers(self, **kwargs):
        ...
        if vrf == 'default':
            ...
            feature = feature_tmp.format('_af_ipv6_neighbor_addr')
        if delete:
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, vrf=vrf, afi=afi,
                                      n_addr=peer_ip, op='_delete')
            return callback(config)
        if get:
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, vrf=vrf, afi=afi,
                                      n_addr=peer_ip, resource_depth=2, op='_get')
            ret = callback(config, handler='get_config')
        ...
        args = dict(min_tx=min_tx, min_rx=min_rx, multiplier=multiplier)
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, vrf=vrf, afi=afi,
                                  n_addr=peer_ip, op='_update', args=args)
        return callback(config)
    ...
    def vni_add(self, **kwargs):
        ...
        if not get_config:
            ...
            if not delete:
                ...
                method_name = ['rbridge_id_evpn_instance_vni_evpn_vni_delete',
