...
class BGP(object):
    ...
    def _bgp_vrf_remote_as(self, rbridge_id, afi, vrf, addr, remote_as):
        ...
        bgp_vrf_remote_as = bgp_vrf_remote_as.format(addr=addr, remote_as=remote_as)
    ...
    def local_asn(self, **kwargs):
        ...
        if is_get_config:
            ...
            api = 'rbridge_id_get'
            config = util.get_bgp_api(api=api, vrf=vrf, op='_get', afi=afi, rbridge_id=rbridge_id, args=args)
        ...
        config = util.get_bgp_api(vrf=vrf, afi=afi, rbridge_id=rbridge_id, args=args)
    ...
    def remove_bgp(self, **kwargs):
        ...
        callback = kwargs.pop('callback', self._callback)
        afi = kwargs.pop('afi', None)
        config = util.get_bgp_api(vrf=vrf, afi=afi, rbridge_id=rbridge_id, op='_delete')
        return callback(config)
    ...
    def neighbor(self, **kwargs):
        ...
        afi = 'ipv4' if ip_addr.version is 4 else 'ipv6'
        n_addr = str(ip_addr.ip)
        if delete:
            raise NotImplementedError
        if not delete and remote_as is None:
            raise ValueError('When configuring a neighbor, you must specify its remote-as.')
        ret = self._neighbor_ip_address(afi, n_addr, rbridge_id, remote_as, vrf, callback, 'create')
        return ret

    def _neighbor_ip_address(self, afi = 'ipv4', n_addr = None, rbridge_id = 1, remote_as = '1', vrf = 'default', callback = None, op = 'create'):
        if afi == 'ipv4':
            if vrf == 'default':
                self._neighbor_ipv4_address(afi, n_addr, rbridge_id, remote_as, callback, op)
            else:
                self._neighbor_ipv4_vrf_address(afi, n_addr, rbridge_id, remote_as, vrf, callback, op)
        if afi == 'ipv6':
            if vrf == 'default':
                self._neighbor_ipv6_address(afi, n_addr, rbridge_id, remote_as, callback, op)
            else:
                self._neighbor_ipv6_vrf_address(afi, n_addr, rbridge_id, remote_as, vrf, callback, op)

    def _neighbor_ipv4_address(self, afi = 'ipv4', n_addr = None, rbridge_id = 1, remote_as = '1', callback = None, op = 'create'):
        args = dict(rbridge_id=rbridge_id, neighbor_addr=n_addr)
        api = 'rbridge_id_router_bgp_neighbor_neighbor_addr_create'
        config = (api, args)
        ret = callback(config)
        args = dict(rbridge_id=rbridge_id, neighbor_addr=n_addr, remote_as=remote_as)
        ...
        config = util.get_bgp_api(n_addr=n_addr, afi=afi, feature='_neighbor_af_ipv4_neighbor_address', rbridge_id=rbridge_id, args=args)
        return callback(config)

    def _neighbor_ipv6_address(self, afi = 'ipv6', n_addr = None, rbridge_id = 1, remote_as = '1', callback = None, op = 'create'):
        self._bgp_vrf_remote_as(rbridge_id, afi, 'default', n_addr, remote_as)
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, afi=afi, feature='_neighbor_af_ipv6_neighbor_address', rbridge_id=rbridge_id, args=args)
        return callback(config)

    def _neighbor_ipv4_vrf_address(self, afi = 'ipv4', n_addr = None, rbridge_id = 1, remote_as = '1', vrf = None, callback = None, op = 'create'):
        args = dict(rbridge_id=rbridge_id, af_vrf=vrf)
        api = 'rbridge_id_router_bgp_address_family_ipv4_unicast_vrf_create'
        config = (api, args)
        ret = callback(config)
        args = dict(rbridge_id=rbridge_id, af_vrf=vrf, af_ipv4_neighbor_addr=n_addr)
        api = 'rbridge_id_router_bgp_address_family_ipv4_unicast_vrf_neighbor_af_ipv4_neighbor_addr_create'
        config = (api, args)
        ret = callback(config)
        args = dict(rbridge_id=rbridge_id, af_vrf=vrf, af_ipv4_neighbor_addr=n_addr, remote_as=remote_as)
        api = 'rbridge_id_router_bgp_address_family_ipv4_unicast_vrf_neighbor_af_ipv4_neighbor_addr_update'
        config = (api, args)
        ret = callback(config)
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, vrf=vrf, afi=afi, feature='_neighbor_af_ipv4_neighbor_addr', rbridge_id=rbridge_id, args=args)
        return callback(config)

    def _neighbor_ipv6_vrf_address(self, afi = 'ipv6', n_addr = None, rbridge_id = 1, remote_as = '1', vrf = None, callback = None, op = 'create'):
        self._bgp_vrf_remote_as(rbridge_id, afi, vrf, n_addr, remote_as)
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, vrf=vrf, afi=afi, feature='_neighbor_af_ipv6_neighbor_addr', rbridge_id=rbridge_id, args=args)
        return callback(config)

    def _neighbor_activate(self, n_addr, afi, rbridge_id, vrf, callback):
        feature = '_neighbor_af_{afi}_neighbor_address'.format(afi=afi)
        args = dict(activate=True)
        config = util.get_bgp_api(n_addr=n_addr, afi=afi, rbridge_id=rbridge_id, feature=feature, vrf=vrf, args=args)
        return callback(config)
    ...
    def get_bgp_neighbors(self, **kwargs):
        ...
        result = []
        if vrf is 'default':
            config = util.get_bgp_api(rbridge_id=rbridge_id, op='_get', resource_depth=2, feature='_neighbor_neighbor_addr')
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi='ipv4', vrf=vrf, op='_get', feature=feature, resource_depth=2, args=dict())
        output = callback(config, handler='get_config')
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi='ipv6', vrf=vrf, op='_get', feature=feature, resource_depth=2, args=dict())
        output = callback(config, handler='get_config')
    ...
    def redistribute(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_get', feature=feature, args=dict())
            output = callback(config, handler='get_config')
            output = util.findall(output.json, '$..redistribute-connected')
            return output
        if kwargs.pop('delete', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_delete', feature=feature, args=dict())
            return callback(config)
        if source is not 'connected':
            raise AttributeError('Invalid source.')
        args = dict(redistribute_connected=True)
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_update', feature=feature, args=args)
        return callback(config)
    ...
    def max_paths(self, **kwargs):
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, args=args)
        return callback(config)
    ...
    def recursion(self, **kwargs):
        ...
        if afi not in ('ipv4', 'ipv6'):
            raise AttributeError('Invalid AFI.')
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_get')
            output = callback(config, handler='get_config')
        ...
        if delete:
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_delete', args=args)
            return callback(config)
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, args=args)
        return callback(config)
    ...
    def graceful_restart(self, **kwargs):
        ...
        if afi not in ('ipv4', 'ipv6'):
            raise AttributeError('Invalid AFI.')
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, op='_get')
            output = callback(config, handler='get_config')
            output = util.findall(output.json, '$..graceful-restart')
            return output
        if kwargs.pop('delete', False):
            args = dict(graceful_restart_status=False)
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, op='_update', args=args)
            return callback(config)
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, args=args)
        return callback(config)
    ...
    def multihop(self, **kwargs):
        ...
        args = dict(ebgp_multihop_flag=True, ebgp_multihop_count=count, rbridge_id=rbridge_id)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, n_addr=str(ip_addr.ip), op='_get')
            output = callback(config, handler='get_config')
            output = util.findall(output.json, '$..ebgp-multihop-count')
            return output
        if kwargs.pop('delete', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, n_addr=str(ip_addr.ip), op='_delete')
            return callback(config)
        config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, n_addr=str(ip_addr.ip), args=args)
        return callback(config)
    ...
    def update_source(self, **kwargs):
        ...
        if vrf == 'default':
            ...
            feature = feature_tmp.format('_af_ipv6_neighbor_addr')
        if int_type is 'loopback':
            args = dict(loopback=int_name)
            feature += '_loopback'
        else:
            args = dict(interface_type=int_type, ethernet=str(int_name), rbridge_id=rbridge_id)
            feature += '_ethernet'
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, feature=feature, n_addr=str(ip_addr.ip), args=args, op='_get')
            ret = callback(config)
            import pdb
            pdb.set_trace()
            return ret
    ...
    def evpn_afi(self, **kwargs):
        ...
        if kwargs.pop('delete', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, op='_delete')
            return callback(config)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, op='_get')
            ret = callback(config, handler='get_config')
            ret = True if ret and ret.json else False
            return ret
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, op='_create')
        return callback(config)
    ...
    def evpn_afi_peer_activate(self, **kwargs):
        ...
        rbridge_id = kwargs.pop('rbridge_id', '1')
        callback = kwargs.pop('callback', self._callback)
        feature = '_address_family_l2vpn_evpn_neighbor_evpn_neighbor_ipv4'
        if kwargs.pop('delete', False):
            args = dict(activate=False)
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=peer_ip, op='_update', args=args)
            return callback(config)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=peer_ip, resource_depth=2, op='_get')
            ret = callback(config, handler='get_config')
            ret = util.find(ret.json, '$..activate')
            return ret
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=peer_ip, op='_create')
        ret = callback(config)
        args = dict(activate=True)
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=peer_ip, op='_update', args=args)
        return callback(config)
    ...
    def bfd(self, **kwargs):
        ...
        if delete:
            ...
            return callback(config)
        if get:
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, resource_depth=2, op='_get')
            ret = callback(config, handler='get_config')
        ...
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, op='_update', args=args)
        return callback(config)
    ...
    def retain_rt_all(self, **kwargs):
        ...
        if kwargs.pop('delete', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, afi=afi, op='_delete')
            return callback(config)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, resource_depth=2, afi=afi, op='_get')
            ret = callback(config, handler='get_config')
            ret = util.find(ret.json, '$..all')
            return ret
        args = dict(all=True)
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, afi=afi, op='_update', args=args)
        return callback(config)
    ...
    def evpn_next_hop_unchanged(self, **kwargs):
        ...
        if kwargs.pop('delete', False):
            ...
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, afi=afi, op='_update', evpn_n_addr=ip_addr, args=args)
            return callback(config)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=ip_addr, afi=afi, op='_get')
    ...
    def evpn_allowas_in(self, **kwargs):
        ...
        if kwargs.pop('delete', False):
            ...
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, afi=afi, op='_delete', evpn_n_addr=ip_addr)
            return callback(config)
        if kwargs.pop('get', False):
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, evpn_n_addr=ip_addr, afi=afi, op='_get', resource_depth=2)
            out = callback(config, handler='get_config')
    ...
    def af_ip_allowas_in(self, **kwargs):
        ...
        if vrf == 'default':
            if 'ipv4' == afi:
                feature = feature_tmp.format('_address_family_ipv4_unicast', '_af_ipv4_neighbor_address')
            elif 'ipv6' == afi:
                feature = feature_tmp.format('_address_family_ipv6_unicast', '_af_ipv6_neighbor_address')
            afi = None
        elif 'ipv4' == afi:
            feature = feature_tmp.format('', '_af_ipv4_neighbor_addr')
        elif 'ipv6' == afi:
            feature = feature_tmp.format('', '_af_ipv6_neighbor_addr')
        if kwargs.pop('get', False):
            config = util.get_bgp_api(vrf=vrf, n_addr=str(ip_addr), rbridge_id=rbridge_id, op='_get', afi=afi, feature=feature, resource_depth=2)
            out = callback(config, handler='get_config')
            ret = util.findall(out.json, '$..allowas-in')[0]
            return ret
        if kwargs.pop('delete', False):
            feature = feature + '_allowas_in'
            config = util.get_bgp_api(vrf=vrf, n_addr=str(ip_addr), rbridge_id=rbridge_id, afi=afi, feature=feature)
            return callback(config)
        args = dict(allowas_in=allowas_in)
        config = util.get_bgp_api(vrf=vrf, n_addr=str(ip_addr), rbridge_id=rbridge_id, op='_update', afi=afi, feature=feature, args=args)
        return callback(config)
    ...
    def peer_bfd_timers(self, **kwargs):
        ...
        if vrf == 'default':
            ...
            feature = feature_tmp.format('_af_ipv6_neighbor_addr')
        if delete:
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, vrf=vrf, afi=afi, n_addr=peer_ip, op='_delete')
            return callback(config)
        if get:
            config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, vrf=vrf, afi=afi, n_addr=peer_ip, resource_depth=2, op='_get')
            ret = callback(config, handler='get_config')
            ...
            return item
        args = dict(min_tx=min_tx, min_rx=min_rx, multiplier=multiplier)
        config = util.get_bgp_api(rbridge_id=rbridge_id, feature=feature, vrf=vrf, afi=afi, n_addr=peer_ip, op='_update', args=args)
        return callback(config)
    ...
    def vni_add(self, **kwargs):
        ...
        if not get_config:
            ...
            if not delete:
                method_name = ['rbridge_id_evpn_instance_create', 'rbridge_id_evpn_instance_vni_evpn_vni_create']
            else:
                method_name = ['rbridge_id_evpn_instance_vni_evpn_vni_delete', 'rbridge_id_evpn_instance_delete']
            for i in range(0, 2):
                method = method_name[i]
                vni_args = dict(rbridge_id=rbridge_id, evpn_instance=evpn_instance)
                if 'vni' in method:
            ...
            vni_args = dict(rbridge_id=rbridge_id, evpn_instance=evpn_instance, resource_depth=2)
    ...
    def vrf_unicast_address_family(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            ...
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_delete')
            result = callback(config)
        else:
            config = util.get_bgp_api(rbridge_id=rbridge_id, afi=afi, vrf=vrf, op='_create')
            result = callback(config)
        return result
