...
class Interface(object):
    ...
    def access_vlan(self, inter_type, inter, vlan_id):
        ...
        """
        # NOT_WORKING
        method_name = 'interface_%s_switchport_access_vlan_create' % inter_type
        kwargs = {inter_type: inter, 'vlan': (vlan_id, '1')}
        config = (method_name, kwargs)
    ...
    def ip_address(self, **kwargs):
        ...
        callback = kwargs.pop('callback', self._callback)
        valid_int_types = ['ethernet', 'gigabitethernet', 'tengigabitethernet', 've',
    ...
    def admin_state(self, **kwargs):
        ...
        try:
            if get:
                ...
                if enabled:
                    state_args['shutdown'] = False
                else:
                    state_args['shutdown'] = True
    ...
    def tag_native_vlan(self, **kwargs):
        ...
        if get:
            ...
            if util.find(x.json, '$..native-vlan'):
                return True
            return None
    ...
    def ip_mtu(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            ...
            return util.find(op.json, '$..ip..mtu')
    ...
    def switchport(self, **kwargs):
        ...
        enabled = kwargs.pop('enabled', True)
        callback = kwargs.pop('callback', self._callback)
        int_types = ['ethernet', 'gigabitethernet', 'tengigabitethernet',
                     'fortygigabitethernet', 'hundredgigabitethernet',
                     'port_channel', 'vlan']
    ...
    def vrrpe_spf_basic(self, **kwargs):
        ...
        if get:
            if version == 4:
                method_name = 'interface_%s_vrrp_extended_group_short_path_forwarding_get' % \
                              int_type
                vrid_name = 'vrrpe'
            else:
                method_name = 'interface_%s_ipv6_vrrp_extended_group_short_path_forwarding_get' % \
                              int_type
    ...
    def vrrpe_vmac(self, **kwargs):
        ...
        if version == 4:
            ...
            method_name = 'interface_%s_ipv6_vrrp_extended_group_virtual_mac_' % int_type
            vrid_name = 'vrrpv3e_group'
    ...
    def create_ve(self, **kwargs):
        ...
        get = kwargs.pop('get', False)
        callback = kwargs.pop('callback', self._callback)
        ve_args = dict(rbridge_id=rbridge_id)
        if get:
            enable = None

        method_name = 'rbridge_id_interface_ve_'

        if get:
            config = ('rbridge_id_get', ve_args)

            output = callback(config, handler='get_config')
            return util.findall(output.json, '$..interface..Ve..name')
    ...
    def create_loopback(self, **kwargs):
        ...
        method_name = 'rbridge_id_interface_loopback_'
        ...
        if get:
            ...
            return util.findall(output.json, '$..interface..Loopback..id')
    ...
    def vrf(self, **kwargs):
        ...
        if not get_config:
            ...
            config = ('rbridge_id_vrf_create', vrf_args)

            if delete:
                config = ('rbridge_id_vrf_delete', vrf_args)
            result = callback(config)

        elif get_config:

            vrf_args = dict(rbridge_id=rbridge_id)
            config = ('rbridge_id_get', vrf_args)
            output = callback(config, handler='get_config')
    ...
    def vrf_route_distiniguisher(self, **kwargs):
        ...
        if not get_config:
            vrf_name = kwargs['vrf_name']
            rd = kwargs['rd']
            rd_args = dict(rbridge_id=rbridge_id, vrf=vrf_name,
                           route_distiniguisher=rd)

            config = ('rbridge_id_vrf_update', rd_args)

            if delete:
                config = ('rbridge_id_vrf_delete', rd_args)
            ...
            config = ('rbridge_id_vrf_get', rd_args)
            output = callback(config, handler='get_config')
            import pprint
            pprint.pprint(output.json)
