...
class InterfaceVRFTestCase(unittest.TestCase):
    ...
    def test_vrf_afi_ipv6(self):
        with Device(conn=self.conn, auth=self.auth) as dev:
            dev.interface.vrf_afi(vrf_name=self.vrf, afi='ipv6', rbridge_id=self.rbridge_id)

            op = dev.interface.vrf_afi(vrf_name=self.vrf, rbridge_id=self.rbridge_id, get=True)
            self.assertEqual({'ipv4': False, 'ipv6': True}, op)

            dev.interface.vrf_afi(vrf_name=self.vrf, afi='ipv6', rbridge_id=self.rbridge_id, delete=True)
    ...
    def test_vrf_afi_rt_evpn(self):
        with Device(conn=self.conn, auth=self.auth) as dev:
            ...
            output = dev.interface.vrf_afi_rt_evpn(
                vrf_name=self.vrf, afi=self.afi, rbridge_id=self.rbridge_id, get=True)
            self.assertIn({'afi': self.afi, 'rbridge_id': self.rbridge_id, 'rt': [
                          self.rt], 'rtvalue': [self.rt_value], 'vrf_name': self.vrf}, output)
            ...
            output = dev.interface.vrf_afi_rt_evpn(
                vrf_name=self.vrf, afi=self.afi, rbridge_id=self.rbridge_id, get=True)
            self.assertNotIn({'afi': self.afi, 'rbridge_id': self.rbridge_id, 'rt': [
                self.rt], 'rtvalue': [self.rt_value], 'vrf_name': self.vrf}, output)
