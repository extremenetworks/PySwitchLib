...
class ServicesTestCase(unittest.TestCase):
    ...
    def test_vrrp(self):
        with Device(conn=self.conn, auth=self.auth) as dev:
            dev.services.vrrp(rbridge_id=self.rbridge_id, enabled=True)
            dev.services.vrrp(rbridge_id=self.rbridge_id, ip_version='6', enabled=True)
            ...
            dev.services.vrrp(rbridge_id=self.rbridge_id, ip_version='6', enabled=False)
            op = dev.services.vrrp(rbridge_id=self.rbridge_id, get=True)
