...
class InterfaceVirtualPortTestCase(unittest.TestCase):
    ...
    def test_create_loopback(self):
        with Device(conn=self.conn, auth=self.auth) as dev:
            ...
            output = dev.interface.create_loopback(
                lb_name=self.loopback_id, rbridge_id=self.rbridge_id, enable=False)
            output = dev.interface.create_loopback(
                get=True, rbridge_id=self.rbridge_id)
    ...
    def test_ip_anycast_gateway(self):
        ...
        with Device(conn=self.conn, auth=self.auth) as dev:
            ...
            output = dev.interface.ip_anycast_gateway(
                rbridge_id=self.rbridge_id, int_type='ve', name=self.vlan, anycast_ip=self.anycast_ip_address)
            output = dev.interface.ip_anycast_gateway(
                rbridge_id=self.rbridge_id, int_type='ve', name=self.vlan,get=True)
    ...
    def test_ipv6_anycast_gateway(self):
        ...
        with Device(conn=self.conn, auth=self.auth) as dev:
            ...
            output = dev.interface.ip_anycast_gateway(
                rbridge_id=self.rbridge_id, int_type='ve', name=self.vlan, anycast_ip=self.anycast_ipv6_address)
            output = dev.interface.ip_anycast_gateway(
                rbridge_id=self.rbridge_id, int_type='ve', name=self.vlan, get=True)
